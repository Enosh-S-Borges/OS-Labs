...
void main() { 
    ...
    while (completed != n) { 
        ...
        if (minIndex == -1) { 
            ...
            idleTime++; 
        } else { // As the process is not preempted, the process executes till it is completed, i.e., for it's full BT. 
            if (idleTime > 0) { 
    ...
    avg_WT = (float)total_WT / n; // Printing the table of processes with details 
